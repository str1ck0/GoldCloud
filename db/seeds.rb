...
rated_items = Hash.new { |h,k| h[k] = Set.new }

Order.delivered.each do |order|
  order.order_items.each do |order_item|
    # Skip if user already rated this item
    next if rated_items[order.user.id].include?(order_item.itemable.item.id)
    ...
    rated_items[order.user.id].add(order_item.itemable.item.id)
  end
end